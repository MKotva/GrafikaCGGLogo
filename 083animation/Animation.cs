...
namespace _083animation
{
  ...
  public class Disc
  {
    public double cx;
    public double cy;
    public double cx_random;
    public double cy_random;
    public double radius;
    public double actual_radius;
    public double showTime;
    public double growingEndTime;
    public Color color;
    public int actual_black_level;
  ...
  public class Animation
  {
    ...
    public static void InitAnimation (int width, int height, double start, double end, double fps, string param)
    {
      ...
      var rnd = new Random();
      discs.OrderBy(_ => Guid.NewGuid()); //Shuffle list.
      double discTimes = ((end - start) / 4) / 200;
      for(int i = 0; i < discs.Count; i++)
      {
        discs[i].showTime = discTimes * i;
        discs[i].growingEndTime = discs[i].showTime + ((end - start) / 2);
      }
    ...
    public static void DrawFrame (Canvas c, double time, double start, double end, string param)
    {
      ...
      double firstAnimationLenght = (end - start) / 4;
      if ((firstAnimationLenght * 3) > (time - start))
      {
        newDiscs = AnimateFirstPart(newDiscs, c, time, start, end, firstAnimationLenght, param);

      }
      else if (end > time - start)
      { 
        newDiscs = AnimateSecondPart(newDiscs, c, time, firstAnimationLenght * 3, end, firstAnimationLenght, param);
      }

      foreach (Disc d in newDiscs)
      {
        c.SetColor(Color.FromArgb(d.actual_black_level));
        c.FillDisc(X(d.cx), Y(d.cy), (float)(d.actual_radius * kxy));
      }

      // !!!}}
    }

    public static List<Disc> AnimateFirstPart (List<Disc> newDiscs, Canvas c, double time, double start, double end, double animationTime, string param)
    {
      for (int j = 0; j < discs.Count; j++)
      {
        if ((time - start) >= discs[j].showTime)
        {
          var disc = new Disc(discs[j].cx, discs[j].cy, discs[j].radius, discs[j].color);
          disc.growingEndTime = discs[j].growingEndTime;
          newDiscs.Add(disc);
        }
      }

      foreach (var disc in newDiscs)
      {
        if (disc.growingEndTime > time)
        {
          var actual_radius = (((disc.radius * 1.3) / 1000000) * ((time - disc.showTime) / ((disc.growingEndTime - disc.showTime) / 1000000)));
          var actual_black_level = (int)(((disc.color.ToArgb()) * (disc.growingEndTime - time)));
          disc.actual_radius = actual_radius;
          disc.actual_black_level = actual_black_level;

          if(actual_radius > disc.radius * 1.3)
          {
            disc.actual_radius = disc.radius * 1.3;
          }
        ...
        {
          disc.actual_radius = (disc.radius * 1.3);
          disc.actual_black_level = (int)(((disc.color.ToArgb())));
        }
      }

      if(newDiscs.Count != 0)
      {
        newDiscs = newDiscs.OrderBy(x => x.actual_radius).ToList();
      }
      return newDiscs;
    }
    public static List<Disc> AnimateSecondPart (List<Disc> newDiscs, Canvas c, double time, double start, double end, double animationTime, string param)
    {
      for (int i = 0; i < discs.Count; i++)
      {
        var disc = new Disc(discs[i].cx, discs[i].cy, discs[i].radius, discs[i].color);
        newDiscs.Add(disc);
        var shrinkSize = (((disc.radius * 0.3) / 100000) * ((time - start) / ((end - start) / 100000)));
        disc.actual_radius = (discs[i].radius * 1.3) - shrinkSize;
        disc.actual_black_level = (int)(disc.color.ToArgb());
      }
